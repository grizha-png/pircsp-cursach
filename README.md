...
- регистрацию, авторизацию и аутентификацию;
...
Серверная часть построена по упрощенной многослойной схеме:

- `src/app/modules/` — доменные модули `auth`, `users`, `tests`, `attempts` с разнесенными роутерами;
- `src/app/modules/*/service.py` — бизнес-логика по каждому доменному модулю;
- `src/app/modules/auth/dependencies.py` — аутентификация, текущий пользователь и проверки ролей;
- `src/app/api/` — общие FastAPI-зависимости, включая подключение к БД;
- `src/app/validation/` — валидация входных данных через Pydantic;
- `src/app/services.py` — совместимый фасад для сервисного слоя и старых импортов;
- `src/app/db/` — схема SQLite, создание соединений и SQL-функции доступа к данным;
- `src/app/core/` — конфигурация приложения и функции безопасности;
- `src/app/seed.py` — заполнение тестовыми данными.

Файлы `src/app/config.py`, `src/app/database.py`, `src/app/repositories.py`, `src/app/dependencies.py`, `src/app/schemas.py` и `src/app/routers/` оставлены как совместимые точки импорта и перенаправляют на новую структуру.

## Функциональность

- регистрация студента и вход в систему по логину и паролю;
